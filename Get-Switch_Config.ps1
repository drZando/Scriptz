$i = @(
    @{Adraesse="sw01";Protokoll="Telnet";Befaeu="bnutzer","en","pass","terminal length 0","sho run"}
    @{Adraesse="sw02";Protokoll="O_SCP";Benutzer="manager";Konfigfile="/cfg/running-config"}
)

$BackupRootOrdner = "\\server\uncpfad\Switches\Backup\"
$Jetz = Get-Date -Format yyyyMMdd-HHmm
$scp_openssh = "C:\Windows\System32\OpenSSH\scp.exe"
...
foreach($switch in $i){
    $KonfigOrdner = ($BackupRootOrdner+$switch.Adraesse).ToString()
    $KonfigDateiname = $switch.Adraesse+"_"+$jetz+".txt"
    if(!(Test-Path $KonfigOrdner -PathType Container)){
        New-Item -Path $KonfigOrdner -ItemType Directory | Out-Null
    }
    $KonfigPfad = $KonfigOrdner+"\"+$KonfigDateiname

    if($switch.Protokoll -eq "Telnet"){
        Get-Telnet -RemoteHost $switch.Adraesse -Commands $switch.Befaeu -OutputPath $KonfigPfad -WaitTime 2000
    }
    elseif($switch.Protokoll -eq "O_SCP"){
	$cmd = $switch.Benutzer + "@" + $switch.Adraesse + ":" + $switch.Konfigfile + " " + $KonfigPfad
        Start-Process -FilePath $scp_openssh -ArgumentList $cmd
    }
}
